...
public class ReservaService {
    ...
    public ReservaService(ReservaRepository repo, UsuarioRepository usuarioRepo, PlatoRepository platoRepo,
            MenuDelDiaRepository menuRepo) {
        this.repo = repo;
    ...
    public Reserva crear(Reserva reserva, String correoUsuario) {
        ...
        if (tieneMenu) {
            ...
            // Validar que la entrada y fondo elegidos existan en el menú
            String entrada = reserva.getEntradaElegida();
            String fondo = reserva.getFondoElegido();

            if (entrada == null || fondo == null) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Debes seleccionar una entrada y un fondo.");
            }

            if (!menu.getEntradas().contains(entrada) || !menu.getFondos().contains(fondo)) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                        "Las opciones elegidas no pertenecen al menú del día.");
            }
